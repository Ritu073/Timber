...
const float AXE_POSITION_LEFT=720;
...
//sound outoftime
SoundBuffer ooBuffer;
ooBuffer.loadFromFile("sound/out_of_time.wav");
Sound outOfTime;
outOfTime.setBuffer(ooBuffer);
...
spriteRIP.setPosition(580,2000);
spritePlayer.setPosition(580,700);
acceptInput = true;
...
srand((int) time(0) * 17);
float height= (rand()%140) + 100; //500 to 1000 range
...
if(!aeroActive){
srand((int)time(0));
aeroSpeed=(rand()% 250)+100; //200-400 speed range
srand((int) time(0) * 20);
float height= (rand()%100) + 100; //500 to 1000 range
